...
class RegressionHead(nn.Module):
    def __init__(self, input_dim=384, output_dim=1) -> None:
        super().__init__()
    ...
    def forward(self, x):
        x = x / 10
        return self.linear(x).sigmoid() * 1.5 + 0.2

if __name__ == '__main__':
    g = GeometryAwareRelationalGraphNeuralNetwork(input_dim=21, hidden_dims=[512, 512, 512, 512, 512, 512], 
                                                                num_relation=7, edge_input_dim=59, num_angle_bin=8,
                                                                batch_norm=True, concat_hidden=True, short_cut=True, readout="sum")
    # m = torch.load('/home/jiahao_chen/ProtText/latest_mc_gearnet_edge.pth')['model']
    # new_keys = {}

    # for k, v in m.items():
    #     if 'mlp' in k or 'spatial' in k:
    #         continue
    #     else:
    #         new_k = k[12:]
    #     new_keys[new_k] = v
    # g.load_state_dict(new_keys)